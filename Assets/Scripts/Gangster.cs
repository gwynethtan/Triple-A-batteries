...
public class Gangster : MonoBehaviour
{
    ...
    /// </summary>
    private NavMeshAgent gangsterAgent;
    ...
    private void Start()
    {
        ...
        gangsterAgent = GetComponent<NavMeshAgent>(); // Get the NavMeshAgent component

        // Ensure the NavMeshAgent is enabled and placed on a valid NavMesh
        if (gangsterAgent != null && gangsterAgent.isOnNavMesh)
        {
            gangsterAgent.enabled = true;
        ...
        currentState = "Patrol"; // Start with idle state 
